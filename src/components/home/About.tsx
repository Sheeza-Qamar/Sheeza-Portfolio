...
export const About = () => {
    const data = [
        ...
        { value: 2, text: "years of experience" },
    ];
    return (
        <section
            id="about"
        ...
        >
            <Layout className="!pt-16">
                <div className="grid w-full grid-cols-8 sm:gap-16 gap-8">
                    <div className="flex justify-center col-span-8 flex-row items-center md:order-1 order-3 gap-16">
                        {data.map((item, index) => (
                            <div key={index} className="flex flex-col items-center justify-center">
                                <span className="inline-block font-bold md:text-7xl sm:text-6xl text-4xl">
                                    <AnimateNumber value={item.value} />+
                                </span>
                                <h2 className="md:text-xl font-medium capitalize text-dark/75 dark:text-light/75 text-center sm:text-lg text-sm">
                                    {item.text}
                                </h2>
                            </div>
                        ))}
